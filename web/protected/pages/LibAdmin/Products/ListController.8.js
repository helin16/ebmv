	...
	,_getResultTableRow: function(row, isTitle) {
		...
		tmp.row = new Element('tr', {'class': 'prodcut-row'}).store('data', row)
			...
			.insert({'bottom': new Element(tmp.tag).update(row.title) 
				.insert({'bottom': new Element('div').update(new Element('small').update(row.description)) 
			})
			.insert({'bottom': new Element(tmp.tag, {'class': 'col-sm-1'}).update(row.price) })
				...
				try {
					...
					tmp.result.items.each(function(item) {
						tmp.item = {
								'id': item.id,
								'title': item.title, 
								'descripiton': item.attributes.description ? item.attributes.description[0].attribute : '', 
