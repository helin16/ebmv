...
PageJs.prototype = Object.extend(new FrontPageJs(), {
	...
	,_getItemsRow: function(row, isTitle) {
		...
		tmp.newDiv = new Element('tr', {'class': 'item-row'}).store('data', row)
			...
			.insert({'bottom': new Element(tmp.tag).update(tmp.isTitle === true ? 'Title' : new Element('abbr', {'title': (!row.product.attributes.description ? '' : row.product.attributes.description[0].attribute)})
					.update(row.product.title)
				)
			})
