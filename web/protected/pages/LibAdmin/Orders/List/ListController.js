...
PageJs.prototype = Object.extend(new FrontPageJs(), {
	...
	,_getResultTableRow: function(row, isTitle) {
		...
		tmp.row = new Element('tr', {'class': 'item-row', 'item-id': row.id}).store('data', row)
			...
			.insert({'bottom': new Element(tmp.tag, {'class': 'col-sm-2'}).update(row.created) })
			.insert({'bottom': new Element(tmp.tag, {'class': 'col-sm-2'}).update(row.createdBy.person.fullname) })
